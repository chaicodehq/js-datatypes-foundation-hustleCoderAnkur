...
export function extractRangoliCenter(design, start, end) {
  //  extracts center portion using slice (2 ms)
  //  extracts from beginning
  //  returns "" for non - string design(1 ms)
  //  returns "" if start or end not numbers
...
export function makeRangoliBorder(char, length) {
  ...
  let rep = Math.round(length / char.length);
  let border = char.repeat(rep);
